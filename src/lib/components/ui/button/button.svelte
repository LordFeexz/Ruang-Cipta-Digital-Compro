	...
	export const buttonVariants = tv({
		...
		variants: {
			variant: {
				...
				/* Primary CTA. Replaces the class string that was hand-repeated in 5 files. */
				brand: "bg-brand text-brand-on hover:bg-brand-hover",
				/* Secondary CTA — hairline outline that reads in both themes. */
				brandGhost: "border-border bg-background/60 text-foreground backdrop-blur-md hover:bg-accent hover:border-brand-ring",
			},
			size: {
				...
				/* Marketing CTA size — pill, 48px tall so it clears the 44px touch minimum. */
				xl: "h-12 gap-2 rounded-full px-7 text-base sm:h-[3.25rem] sm:px-8",
				icon: "size-9",
